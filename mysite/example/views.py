...
def fib(request):
	...
	if request.method == "POST":
		try:
			...
			startNumber = int(jsob["startNumber"]) # converts start number into integer
			length = int(jsob["length"]) # converts length into integer
			loop = range(length) # creates a range of numbers
